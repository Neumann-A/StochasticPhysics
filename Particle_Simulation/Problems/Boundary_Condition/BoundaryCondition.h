...
public:
	...
	{
		auto tmp = std::pow(xi(0) / _a, 2) + std::pow(xi(1) / _b, 2) + std::pow(xi(2) / _c, 2);
		...
		if (tmp > 1) // New Vector Outside boundary
		{
			...
			normalatcontact << 2 / std::pow(_a, 2)*contactpoint(0), 2 / std::pow(_b, 2)*contactpoint(1), 2 / std::pow(_c, 2)*contactpoint(2); //Normal at Boundary contact point

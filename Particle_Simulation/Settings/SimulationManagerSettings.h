...
namespace Settings
{
	...
	public:
		...
		void setSimulationSettings(const SimulationSettings& simulationSettings)	{ _SimulationSettings = simulationSettings;	}

		// Access the ProblemSettings
		const ProblemSettings& getProblemSettings(void) const				{ return(*_pProblemSettings);			}
		void setProblemSettings(const std::unique_ptr<ProblemSettings>& problemSettings)	{ _pProblemSettings = problemSettings;	}

		SimulationManagerSettings(const Provider& provider, const SimulationSettings& sim, const SolverSettings& solver, const ResultSettings& result,
			const ProblemSettings& problem, const FieldProperties& field)
			:  _SimulationSettings(sim), _SolverSettings(solver), _ResultSettings(result), _FieldProperties(field),
			_pProblemSettings(problem.clone()), _pParticleProvider(std::make_unique<Provider>(provider))
		{
		...
		SimulationManagerSettings(const SimulationManagerSettings &tocopy) :
			_SimulationSettings(tocopy._SimulationSettings),_SolverSettings(tocopy._SolverSettings), _ResultSettings(tocopy._ResultSettings),
			_FieldProperties(tocopy._FieldProperties), _pProblemSettings((*tocopy._pProblemSettings).clone()), _pParticleProvider(std::make_unique<Provider>(*tocopy._pParticleProvider)) {};

		SimulationManagerSettings operator=(const SimulationManagerSettings &tocopy)
		{
			return SimulationManagerSettings{ *tocopy._pParticleProvider ,tocopy._SimulationSettings,tocopy._SolverSettings,tocopy._ResultSettings,*tocopy._pProblemSettings,tocopy._FieldProperties };
		}
...
namespace Archives
{
	...
	public:
		...
		static inline type construct(InputArchive<Archive>& ar)
		{
			...
			pParticleProvider = std::make_unique<typename type::Provider>(Archives::LoadConstructor<typename type::Provider>::construct(ar));

			type ConstructedType{*pParticleProvider,SimulationSettings,SolverSettings,ResultSettings,*pProblemSettings,FieldProperties};

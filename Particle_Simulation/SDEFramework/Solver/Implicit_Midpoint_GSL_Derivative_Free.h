...
namespace SDE_Framework::Solvers
{
    ...
    public:
        ...
        auto getResultNextFixedTimestep(const Precision &time, const DependentType &yi, const IndependentFunctor &xifunc)
        {
            ...
            std::cout << "Finished Implicit Solver after " << std::to_string(watch*Timer.unitFactor()) + " s (" << std::to_string(watch / (numberofiter)) << " ns/iteration) Iterations:" << (numberofiter) << std::endl;
#endif
